...
window.onload = async function () {
     ...
     if (response.ok) {
         ...
         for (let i = feed.length-1; i >= 0; i--){
             await applyFeed(feed[i].message, getMonthName(feed[i].createdAt.slice(6, 7)).slice(0, 3) + ' ' + feed[i].createdAt.slice(8, 10), feed[i].username)
             }
     }
 ...
 async function applyFeed(message, date, username) {
     ...
     pfp.src = "http://localhost:3000" + await getProfileUrl(username)
