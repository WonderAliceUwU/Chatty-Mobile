...
async function getFeed(){
    ...
    if (response.ok) {
        ...
        for (let i = feed.length - 1; i >= 0; i--) {
            if (feed[i].imageFilename !== null) {
                await applyFeed(feed[i].message, getMonthName(feed[i].createdAt.slice(6, 7)).slice(0, 3) + ' ' + feed[i].createdAt.slice(8, 10), feed[i].username, feed[i].imageFilename)
            } else {
                await applyFeed(feed[i].message, getMonthName(feed[i].createdAt.slice(6, 7)).slice(0, 3) + ' ' + feed[i].createdAt.slice(8, 10), feed[i].username, null)
            }
        }
    }
}
 async function applyFeed(message, date, username, filename) {
     ...
     pfp.src = "http://"+ localStorage.getItem('server') + await getProfileUrl(username)
